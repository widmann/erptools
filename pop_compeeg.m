...
Arg.colArray = {'Color', [0 0 0], 'LineStyle', '-'; 'Color', [0.5 0 0], 'LineStyle', '-'; 'Color', [0 0.5 0], 'LineStyle', '-'};
...
for iSet = 1:length(EEG)
    if Arg.avg
        ...
        Arg.data = cat(3, Arg.data, reshape(EEG(iSet).data, size(EEG(iSet).data, 1), []));
    end
end
...
Arg.axisPos = [Arg.axisPos(1) Arg.axisPos(2) + 0.05 Arg.axisPos(3) Arg.axisPos(4) - 0.05];
...
Arg.xSliderH = uicontrol('Style', 'slider', 'Units', 'normalized', 'Position', Arg.xSliderPos, 'Callback', @cbslider, 'Min', 1, 'Max', Arg.xSliderMax, 'Value', 1, 'SliderStep', Arg.xSliderStep);
...
function cbslider(src, evt)

    f = get(src, 'Parent');
    Arg = get(f, 'UserData');

    xMin = floor((get(src, 'Value') - 1) / (Arg.xscalePnts / 5)) * (Arg.xscalePnts / 5) + 1;
    set(src, 'Value', xMin);
...
function updateplot(Arg, xMin)
    ...
    evtArray = find([Arg.event.latency] >= xMin & [Arg.event.latency] < xMin + Arg.xscalePnts);
    ...
    for iSet = Arg.nSets:-1:1
        ...
        for iEvt = 1:length(evtArray)
            plot(([Arg.event(evtArray([iEvt iEvt])).latency] - 1) / Arg.srate, [0 Arg.yscale * Arg.nChans], 'r')
            text((Arg.event(evtArray(iEvt)).latency - 1) / Arg.srate, 0, num2str(Arg.event(evtArray(iEvt)).type), 'VerticalAlignment', 'Bottom')
        end

    end
    
    set(Arg.axisH, 'YTick', Arg.yOffsetArray, 'YTickLabel', Arg.chanLabels, 'YLim', [0 Arg.yscale * Arg.nChans], 'XLim', ([xMin xMin + Arg.xscalePnts - 1] - 1) / Arg.srate, 'NextPlot', 'ReplaceChildren')
